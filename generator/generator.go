...
func (g *Generator) genUpdate(t *Table) {
	...
	g.Pn("    }")
	g.Pn("")
	g.Pn("    if rowsAffected!=1{")
	g.Pn("        return fmt.Errorf(\"rowsAffected:%%s\",rowsAffected)")
	g.Pn("    }")
	g.Pn("")
	g.Pn("    return nil")
	g.Pn("}")
...
func (g *Generator) genQuery(t *Table) {
	...
	for _, c := range t.ColumnList {
		g.Pn("func (q *%sQuery)%s_Equal(v %s)*%sQuery{", t.GoName, c.GoName, c.GoType, t.GoName)
		g.Pn("    q.whereBuffer.WriteString(\"%s='\"+fmt.Sprint(v)+\"'\")", c.DbName)
		g.Pn("    return q")
		g.Pn("}")
		g.Pn("")

		g.Pn("func (q *%sQuery)%s_NotEqual(v %s)*%sQuery{", t.GoName, c.GoName, c.GoType, t.GoName)
		g.Pn("    q.whereBuffer.WriteString(\"%s<>'\"+fmt.Sprint(v)+\"'\")", c.DbName)
		g.Pn("    return q")
		g.Pn("}")
		g.Pn("")

		g.Pn("func (q *%sQuery)%s_Less(v %s)*%sQuery{", t.GoName, c.GoName, c.GoType, t.GoName)
		g.Pn("    q.whereBuffer.WriteString(\"%s<'\"+fmt.Sprint(v)+\"'\")", c.DbName)
		g.Pn("    return q")
		g.Pn("}")
		g.Pn("")

		g.Pn("func (q *%sQuery)%s_LessEqual(v %s)*%sQuery{", t.GoName, c.GoName, c.GoType, t.GoName)
		g.Pn("    q.whereBuffer.WriteString(\"%s<='\"+fmt.Sprint(v)+\"'\")", c.DbName)
		g.Pn("    return q")
		g.Pn("}")
		g.Pn("")

		g.Pn("func (q *%sQuery)%s_Greater(v %s)*%sQuery{", t.GoName, c.GoName, c.GoType, t.GoName)
		g.Pn("    q.whereBuffer.WriteString(\"%s>='\"+fmt.Sprint(v)+\"'\")", c.DbName)
		g.Pn("    return q")
		g.Pn("}")
		g.Pn("")

		g.Pn("func (q *%sQuery)%s_GreaterEqual(v %s)*%sQuery{", t.GoName, c.GoName, c.GoType, t.GoName)
		g.Pn("    q.whereBuffer.WriteString(\"%s='\"+fmt.Sprint(v)+\"'\")", c.DbName)
		g.Pn("    return q")
		g.Pn("}")
		g.Pn("")
...
func (g *Generator) gen() {
	...
	g.Pn("    \"time\"")
	g.Pn("    \"context\"")
	g.Pn("    \"github.com/NeuronEvolution/sql/wrap\"")
