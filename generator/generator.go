...
func (g *Generator) genQuery(t *Table) {
	...
	g.Pn("func (q *%sQuery) OrderBy(fields []%s_FIELD, asc bool) *%sQuery {", t.GoName, strings.ToUpper(t.DbName), t.GoName)
	g.Pn("    s:=make([]string,len(fields))")
	g.Pn("    for i,v:=range s{")
	g.Pn("        s[i]=string(v)")
	g.Pn("    }")
	g.Pn("")
	g.Pn("    if asc {")
	g.Pn("        q.order=fmt.Sprintf(\" order by %%s asc\", strings.Join(s,\",\"))")
	g.Pn("    } else {")
	g.Pn("        q.order=fmt.Sprintf(\" order by %%s desc\", strings.Join(s,\",\"))")
	g.Pn("    }")
	g.Pn("")
	g.Pn("    return q")
	g.Pn("}")
	...
	g.Pn("func (q *%sQuery) OrderByGroupCount(left []%s_FIELD,right []%s_FIELD, asc bool) *%sQuery {",
		t.GoName, strings.ToUpper(t.DbName), strings.ToUpper(t.DbName), t.GoName)
	g.Pn("    s:=\" order by \"")
	g.Pn("    if left!=nil{")
	g.Pn("        for _,v:=range left{")
	g.Pn("            s+=string(v)+\",\"")
	g.Pn("        }")
	g.Pn("    }")
	g.Pn("    s+=\"count(1)\"")
	g.Pn("    if right!=nil{")
	g.Pn("        for _,v:=range right{")
	g.Pn("            s+=\",\"+string(v)")
	g.Pn("        }")
	g.Pn("    }")
	g.Pn("")
