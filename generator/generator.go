...
func (g *Generator) genSelectGroupBy(t *Table) {
	g.Pn("func (dao *%sDao)SelectGroupBy(ctx context.Context,tx *wrap.Tx,groupByFields []%s_FIELD,query string)(list []*%s,err error){", t.GoName, strings.ToUpper(t.DbName), t.GoName)
	g.Pn("    querySql:=\"SELECT \"+%s+\" FROM %s \"+query", strings.ToUpper(t.DbName)+"_ALL_FIELDS_STRING", t.DbName)
	g.Pn("    var rows *wrap.Rows")
	g.Pn("    if tx==nil{")
	g.Pn("        rows,err=dao.db.Query(ctx,querySql)")
...
func (g *Generator) genQuery(t *Table) {
	...
	g.Pn("	if asc {")
	g.Pn("	q.order=fmt.Sprintf(\" order by %%s asc\", fieldName)")
	g.Pn("} else {")
	g.Pn("	q.order=fmt.Sprintf(\" order by %%s desc\", fieldName)")
	g.Pn("}")
	g.Pn("")
	g.Pn("	return q")
	g.Pn("}")
